...
* Настройка СЗИ:
* В модуле «Сетевая защита» → «Обнаружение вторжений» (вкладка «Детекторы», пункт «Сканирование портов») Secret Net Studio задается пороговое количество сканирующих пакетов в единицу времени.
* Логика срабатывания:
* При значении порога 950 интенсивности запросов утилиты hping3 достаточно для детекции — система регистрирует на сетевую атаку. При значении порога 1000 чувствительность детектора снижается (загрубляется), и атака не фиксируется.
